...
use super::config::ParserOptLines;
...
pub struct DelimitedLineTokenizer<'rd, 'cfg, R: Read> {
    delim: char,
    enclc: Option<char>,
    parser_opt_lines: &'cfg Option<ParserOptLines>, // needed here to skip lines while iterating
...
impl<'rd, 'cfg, R: Read> DelimitedLineTokenizer<'rd, 'cfg, R> {
    pub fn custom(
        parser_opt_lines: &'cfg Option<ParserOptLines>,
        raw_data: &'rd mut R,
        delim: char,
        enclc: Option<char>,
    ) -> Self {
    ...
    pub fn csv(file_sanitizers: &'cfg Option<ParserOptLines>, raw_data: &'rd mut R) -> Self {
        DelimitedLineTokenizer::custom(file_sanitizers, raw_data, ',', Some('"'))
    }
    pub fn tab(file_sanitizers: &'cfg Option<ParserOptLines>, raw_data: &'rd mut R) -> Self {
        DelimitedLineTokenizer::custom(file_sanitizers, raw_data, '\t', None)
    }
    pub fn default(file_sanitizers: &'cfg Option<ParserOptLines>, raw_data: &'rd mut R) -> Self {
        DelimitedLineTokenizer::csv(file_sanitizers, raw_data)
    }
    ...
    fn skip_file_line_by_file_sanitizer(&self, line_counter: usize, line: &String) -> bool {
        ...
        if let Some(ref sanitize_file) = self.parser_opt_lines {
            sanitize_file
                .transitizers
                .iter()
        ...
        for c in s.chars() {
            state = match state {
                ...
                State::QuotedField => match c {
                    _ if Some(c) == self.enclc => State::QuoteInQuotedField,
                    _ => {
                ...
                State::QuoteInQuotedField => match c {
                    _ if c == self.delim => State::Scan, // enlosure closed, ready for next field
                    _ if Some(c) == self.enclc => {
...
pub struct DelimitedLineTokenizerIterator<'rd, 'cfg, R: Read> {
    dlt: DelimitedLineTokenizer<'rd, 'cfg, R>,
    stats: DelimitedLineTokenizerStats,
}

impl<'rd, 'cfg, R: Read> DelimitedLineTokenizerIterator<'rd, 'cfg, R> {
    pub fn get_stats(&self) -> &DelimitedLineTokenizerStats {
        &self.stats
    }
}

impl<'rd, 'cfg, R: Read> Iterator for DelimitedLineTokenizerIterator<'rd, 'cfg, R> {
...
impl<'rd, 'cfg, R: Read> IntoIterator for DelimitedLineTokenizer<'rd, 'cfg, R> {
    type Item = (usize, Result<(Vec<String>, DelimitedLineTokenizerStats)>);
    type IntoIter = DelimitedLineTokenizerIterator<'rd, 'cfg, R>;
...
mod tests {
    ...
    fn empty() {
        ...
        let dlt = DelimitedLineTokenizer::default(&None, &mut test_data_cursor);
    ...
    fn tab_separated_simple() {
        ...
        let dlt = DelimitedLineTokenizer::tab(&None, &mut test_data_cursor);
        let mut dlt_iter = dlt.into_iter();
        let (_line_num, res) = dlt_iter.next().unwrap();
    ...
    fn tab_separated_simple_enclosed() {
        ...
        let dlt = DelimitedLineTokenizer::custom(&None, &mut test_data_cursor, '\t', Some('"'));
        let mut dlt_iter = dlt.into_iter();
        let (_line_num, res) = dlt_iter.next().unwrap();
    ...
    fn multiple_lines_test_simple() {
        ...
        let dlt = DelimitedLineTokenizer::csv(&None, &mut test_data_cursor);
        let mut dlt_iter = dlt.into_iter();
